...
# y = .9x^3+3x^2+2x+1
y1_3 = (0.9 * x1_1d * x1_1d * x1_1d) + (3 * x1_1d * x1_1d) + (2 * x1_1d) + (1+ noise)
...
#given the 50 x-values and y-values (as defined) find the best coefficents in the form ax^3+bx^2+cx+d, and return an array (degree=3)
coef = np.polyfit(x1_1d, y1_3,3)
...
print()
print('\t##### Output #####')
print('\t2D Data: Intercept: {0} Coefficients: {1}'.format(coef[3], coef[:3]))#the intercept is the last value in the coef array
...
matplotlib.pyplot.scatter(x1_1d, y1_3)

# predict for inputs along the graph to find the best-fit line, plug in all x-values into 3rd degree equation given the coefficients
